...
class Ccc_Vendor_AccountController extends Mage_Core_Controller_Front_Action {
	 ...
	 * @return Mage_vendor_AccountController
	 */
		...
		if ($this->getRequest()->isPost()) {
			...
			if ($vendor->getIsChangePassword()) {
				...
				if (strlen($newPass)) {
					...
					$errors[] = $this->__('New Password fields cannot be empty');
				}
			...
			try {
				$vendor->setCleanPasswordsValidationData();
				$vendor->save();
				...
				$this->_getSession()->setVendor($this->getRequest()->getPost())
					->addException($e, $this->__('Cannot Save Vendor'));
			}
